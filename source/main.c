...
static const char *episodic_lib_dir(void) {
  ...
  if (checked < 0) {
    checked = file_exists("lib/episodic/libclient.so") &&
...
static void load_modules(void) {
  ...
  // We batch the cache flush: one armDCacheFlush + one armICacheInvalidate
  // per module covers every page written above, instead of per-segment.
  for (unsigned int i = 0; i < NUM_MODULES; i++) {
...
static void setup_game_environment(so_module *launcher) {
  ...
  if (!config.vsync) {
    // Mesa honors this env var; the engine also exposes fps_max but the env
    // var is checked earlier and survives GL re-inits inside the engine.
...
// Adaptive clock manager: the original 250 ms / 3 s ring was slow to react
// to scene changes (loading -> gameplay -> menu).  We sample at 100 ms and
// keep a 1.5 s window, which catches 30 -> 60 fps transitions inside a
// second without thrashing the APM mode.  The hysteresis bands are wider
// than the original so brief dips (saving, map transition) don't ping-pong
// the CPU between 1020 MHz and 1785 MHz.
extern volatile unsigned sdl_swap_count;
extern volatile int g_video_playing;

#define CLK_SAMPLE_MS     100
#define CLK_WIN_TICKS     15     // 1.5 s window
#define CLK_BOOST_AT      10     // < ~6 fps -> boost
#define CLK_DROP_AT       45     // > ~30 fps sustained -> release
...
static void *clock_thread(void *arg) {
  ...
  for (;;) {
    svcSleepThread(CLK_SAMPLE_MS * 1000000ll);
    if (g_video_playing) {
      last_swap = sdl_swap_count;
      win_sum = 0;
      for (int i = 0; i < CLK_WIN_TICKS; i++) ring[i] = 0;
      ri = 0;
      continue;
    }

    const unsigned cur = sdl_swap_count;
    unsigned delta = cur - last_swap;
    win_sum -= ring[ri];
    ring[ri] = delta;
    win_sum += ring[ri];
...
static void start_clock_thread(void) {
  ...
  if (pthread_create(&t, &at, clock_thread, NULL) == 0)
    pthread_detach(t); // runs for the lifetime of the process; don't join.
  pthread_attr_destroy(&at);
}

int main(void) {
  // FastLoad gives us 1785 MHz from the very first cycle: critical for
  // module loading (which is memcpy-bound) and the first frame.
  cpu_boost(1);
  ...
  // Fast-boot path: skip the picker entirely if the user enabled it AND
  // the configured game is installed.  Falls through to the picker on any
  // doubt so the user sees a clear status instead of a fatal_error.
  int picker = 1;
  if (config.auto_boot) {
    picker = picker_quick_check();
    if (picker < 0) {
      // Configured game isn't installed; fall back to picker so the user
      // sees a status instead of a fatal_error.
      picker = picker_run();
    }
  } else {
    picker = picker_run();
